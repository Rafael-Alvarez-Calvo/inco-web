import { Wrench, Clock, ClipboardList, Users, type LucideIcon } from 'lucide-react'
...
const FEATS: Feat[] = [
  { Icon: Wrench,        title: 'Rigor técnico',               desc: 'Ingenieros de Caminos, ITOP, Topógrafos, Coordinadores de S&S, Delineantes y Consultores Externos.' },
  { Icon: Clock,         title: 'Plazos garantizados',         desc: '100% de cumplimiento en proyectos de hasta 90M€ y 36 meses de ejecución.' },
  { Icon: ClipboardList, title: 'Visión global',               desc: 'Desde el estudio previo y análisis de viabilidad hasta la Dirección Facultativa y Control de Calidad.' },
  { Icon: Users,         title: 'Clientes públicos y privados', desc: 'Ministerio de Fomento, ADIF, Junta de Extremadura, ayuntamientos y promotores privados.' },
]
...
export const About = () => {
  ...
  return (
    <section id="nosotros" className="py-16 md:py-28 px-4 sm:px-8 lg:px-16 bg-white">
      <div className="max-w-6xl mx-auto">

        {/* ── Texto completo — ancho 100% ── */}
        <div
          ref={headRef}
          className={`transition-all duration-700 mb-10 md:mb-12 ${headIn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-9'}`}
        >
          ...
          {/* Párrafos + cita: apilados en móvil, 2 columnas en md+ */}
          <div className="grid md:grid-cols-2 gap-6 md:gap-10 lg:gap-16 items-start">
            ...
            <blockquote className="pl-6 border-l-4 border-amber bg-amber-light py-5 pr-6 rounded-r-[4px] self-center">
        ...
        {/* ── Cajitas (izq) + Imagen (dcha) — 2 columnas 50/50 ── */}
        <div
          ref={bodyRef}
          className="grid md:grid-cols-2 gap-8 md:gap-10 lg:gap-14 items-start"
        >
          {/* Cajitas */}
            ...
            {FEATS.map(({ Icon, title, desc }, i) => (
              ...
              >
                <div className="flex-shrink-0 w-8 h-8 rounded-[4px] bg-blue-light flex items-center justify-center mt-0.5">
                  <Icon size={16} className="text-blue" strokeWidth={1.75} aria-hidden="true" />
                </div>
                <div>
                  <h4 className="text-[13px] font-semibold text-blue mb-1">{title}</h4>
                  <p className="text-[12px] text-stone-400 leading-snug">{desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Imagen */}
          <div
            className={`relative transition-all duration-700 delay-200 ${bodyIn ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-9'}`}
          >
