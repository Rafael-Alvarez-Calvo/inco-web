...
const company  = ['Quiénes somos', 'Garantías', 'Certificaciones', 'Oficinas', 'Contacto']
const anchors  = ['#nosotros', '#garantias', '#calidad', '#presencia', '#contacto']
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-14 pb-12 border-b border-white/10">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {services.map(s => (
                <li key={s}><button onClick={() => nav('#servicios')} className="text-[13.5px] text-white/65 hover:text-white transition-colors">{s}</button></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {company.map((c, i) => (
                <li key={c}><button onClick={() => nav(anchors[i])} className="text-[13.5px] text-white/65 hover:text-white transition-colors">{c}</button></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <address className="text-[13px] leading-relaxed not-italic">
              C/ Casas de Miravete 22A<br />
              3ª Planta – Madrid<br /><br />
              <a href="tel:+34914994717" className="hover:text-white transition-colors">+34 91 499 47 17</a><br />
              <a href="mailto:info@inco.com.es" className="hover:text-white transition-colors">info@inco.com.es</a>
            </address>
          </div>
        </div>
